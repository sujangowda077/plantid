...
async function queryPlantNet(imageFile, organ) {
  ...
  const timeout = setTimeout(() => controller.abort('plantnet-timeout'), 12000);

  let res;
  try {
    res = await fetch(url, { method: 'POST', body: fd, signal: controller.signal, cache: 'no-store' });
  } catch (error) {
    const msg = error?.name === 'AbortError'
      ? 'PlantNet request timed out. Check your internet connection or firewall and try again.'
      : 'Could not reach PlantNet API. Verify network access to my-api.plantnet.org and try again.';
  ...
  if (!res.ok) {
    ...
    if (res.status >= 500) {
      throw new ApiError(body?.message || `PlantNet upstream error ${res.status}. Try again shortly.`, 503);
    }
    throw new ApiError(body?.message || `PlantNet API error ${res.status}`, 502);
  }
